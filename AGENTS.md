...
Assume other agents may be working in this repo concurrently:

- Do **not** create/apply/drop `git stash` entries unless explicitly requested (includes `git pull --rebase --autostash`). Keep unrelated WIP untouched.
- When the user says “push”, you may `git pull --rebase` (never discard others’ work). “commit” = scope to your changes only. “commit all” = commit everything in grouped chunks.
...
- Focus reports on your edits; avoid guard-rail disclaimers unless truly blocked. End with a brief “other files present” note only if relevant.

## Agent Vocabulary & Signals
...
- “session” file → Pi session logs at `~/.openclaw/agents/<agentId>/sessions/*.jsonl` (use `agent=<id>` from the Runtime line; newest unless specific ID given). SSH via Tailscale for other machines.

## Dependency Rules

- Never edit `node_modules` (global/Homebrew/npm/git installs too). Skill notes go in `tools.md` or `AGENTS.md`.
...
- Lobster seam: use the shared CLI palette in `src/terminal/palette.ts` (no hardcoded colors); apply palette to onboarding/config prompts and other TTY UI output.
- Never send streaming/partial replies to external messaging surfaces (WhatsApp, Telegram); only final replies. Streaming/tool events may go to internal UIs/control channel.
- For manual `openclaw message send` messages that include `!`, use heredoc to avoid Bash escaping.

## Tool Schema Guardrails

- Avoid `Type.Union` in tool input schemas; no `anyOf`/`oneOf`/`allOf`. Use `stringEnum`/`optionalStringEnum` (Type.Unsafe enum) for string lists, and `Type.Optional(...)` instead of `... | null`. Keep top-level schema as `type: “object”` with `properties`.
- Avoid raw `format` property names; some validators treat it as a reserved keyword.

## macOS / iOS / Android Notes

- Gateway currently runs only as the menubar app; no separate LaunchAgent. Restart via the OpenClaw Mac app or `scripts/restart-mac.sh`; verify with `launchctl print gui/$UID | grep openclaw`. **Debug via the app, not ad-hoc tmux sessions; kill temporary tunnels before handoff.**
- macOS logs: `./scripts/clawlog.sh` (unified logs, follow/tail/category filters, expects passwordless sudo for `/usr/bin/log`).
- Do not rebuild the macOS app over SSH; rebuilds must run directly on the Mac.
- SwiftUI: prefer `Observation` framework (`@Observable`, `@Bindable`) over `ObservableObject`/`@StateObject`; migrate existing usages when touching related code.
...
- **Device checks:** verify connected real devices before reaching for simulators/emulators.
- Voice wake: command template stays `openclaw-mac agent --message “${text}” --thinking low` (`VoiceWakeForwarder` handles escaping). Ensure launchd PATH includes standard system paths + pnpm bin (`$HOME/Library/pnpm`).

## Version & Release

- Version locations: `package.json` (CLI), `apps/android/app/build.gradle.kts` (versionName/versionCode), `apps/ios/Sources/Info.plist` + `apps/ios/Tests/Info.plist` (CFBundleShortVersionString/CFBundleVersion), `apps/macos/Sources/OpenClaw/Resources/Info.plist` (CFBundleShortVersionString/CFBundleVersion), `docs/install/updating.md` (pinned npm version), `docs/platforms/mac/release.md` (APP_VERSION/APP_BUILD examples), Peekaboo Xcode projects/Info.plists (MARKETING_VERSION/CURRENT_PROJECT_VERSION).
- “Bump version everywhere” = all locations above **except** `appcast.xml` (only touch for macOS Sparkle releases).
- A2UI bundle hash: `src/canvas-host/a2ui/.bundle.hash` is auto-generated; only regenerate via `pnpm canvas:a2ui:bundle` (or `scripts/bundle-a2ui.sh`). Commit as separate commit.
- Release signing/notary keys managed outside repo; follow internal release docs.
- Notary auth env vars (`APP_STORE_CONNECT_ISSUER_ID`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_API_KEY_P8`) expected in environment.
- Release guardrails: do not change version numbers without operator’s explicit consent; always ask before any npm publish/release step.
- Beta release: use matching npm version suffix (e.g. `YYYY.M.D-beta.N` for tag `vYYYY.M.D-beta.N`); plain versions get consumed/blocked.

## General Agent Behavior

- When adding a new `AGENTS.md` anywhere in the repo, also add a `CLAUDE.md` symlink (`ln -s AGENTS.md CLAUDE.md`).
- When working on a GitHub Issue or PR, print the full URL at the end of the task.
- Respond with high-confidence answers only: verify in code; do not guess.
- Bug investigations: read source of relevant npm dependencies and all related local code before concluding.
- Connection providers: when adding a new connection, update every UI surface and docs (macOS app, web UI, mobile, onboarding/overview docs) + matching status/config forms.
- If shared guardrails are available locally, review them; otherwise follow this repo’s guidance.
- Lint/format churn:
  - If staged+unstaged diffs are formatting-only, auto-resolve without asking.
  - If commit/push already requested, auto-stage formatting-only follow-ups (same or tiny follow-up commit), no extra confirmation.
